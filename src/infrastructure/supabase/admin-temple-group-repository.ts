...
import type { AdminTempleGroupRepository } from '@/domain/repositories/admin-temple-group-repository'
...
export const supabaseAdminTempleGroupRepository: AdminTempleGroupRepository = {
  ...
  async deleteTempleGroup(id) {
    const { error } = await supabase.from('temple_groups').delete().eq('id', id)
